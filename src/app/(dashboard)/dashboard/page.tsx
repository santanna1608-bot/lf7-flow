    ...
    <div className="space-y-8 max-w-[1400px] mx-auto pb-10 relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">Bem-vindo de volta! Aqui está um resumo do seu fluxo de IA.</p>
        </div>
        <button 
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {loadingStats ? (
          ...
          <div key={stat.name} className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 relative group hover:shadow-md transition-all overflow-hidden">
            <div className="flex items-center justify-between mb-8">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{stat.name}</span>
              <div className={cn("p-2 rounded-xl bg-slate-50 border border-slate-100", stat.color)}>
            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-1 mt-3">
                ...
                <span className="text-slate-400 text-[10px] font-bold ml-1 uppercase tracking-tighter">vs mês anterior</span>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 relative overflow-hidden group">
           <div className="flex items-center justify-between mb-8">
             <h3 className="text-xl font-black text-slate-900 tracking-tight">Volume de Mensagens <span className="text-slate-400">(IA vs Humano)</span></h3>
           </div>
             <StatsChart data={chartData} />
        </div>

        {/* Activity Section */}
        <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col relative overflow-hidden group">
           <h3 className="text-xl font-black text-slate-900 mb-8 text-center tracking-tight">Atividade Recente</h3>
             ...
             {activities.length > 0 ? activities.map((activity) => (
               <div key={activity.id} className="flex items-center justify-between group/item">
                 <div className="flex items-center gap-4">
                   <div className={cn(
                     "h-10 w-10 rounded-xl flex items-center justify-center border border-slate-100 transition-all group-hover/item:scale-110",
                     activity.color.includes('emerald') ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-blue-50 text-blue-600 border-blue-100"
                   )}>
                      <UserPlus className="h-5 w-5" />
                   </div>
                   <div className="flex flex-col">
                     <span className="text-sm font-black text-slate-900 leading-tight">{activity.title}</span>
                     <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{activity.time}</span>
                   </div>
                 </div>
                 <span className={cn("text-xs font-black tracking-tighter", activity.color)}>{activity.amount}</span>
               </div>
             )) : (
           ...
           <Link 
             href="/crm"
             className="mt-10 h-12 flex items-center justify-center rounded-xl bg-slate-50 border border-slate-100 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] hover:text-slate-900 hover:bg-slate-100 transition-all w-full"
           >
